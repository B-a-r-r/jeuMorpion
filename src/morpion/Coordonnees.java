...
class Coordonnees {
    ...
    static boolean estDansPlateau(Coordonnees coord, int taille) {
        return 0<=coord.ligne && coord.ligne<taille 
                && 0<=coord.colonne && coord.colonne<taille;
    }
    ...
    static Coordonnees[] voisines(Coordonnees coord, int taille) {
        ...
        Direction[] directions = new Direction[]{Direction.NORD,
                                                 Direction.NORD_EST,
        ...
        for (Direction i: directions) {
            tmp = suivante(coord, i);
            if (estDansPlateau(tmp, taille)) {
                voisines[nbVoisines] = tmp;
                nbVoisines += 1;
            }
        }
